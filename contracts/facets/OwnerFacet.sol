...
contract OwnerFacet is Modifiers {
    ...
    */
    ...
    function createMarket(address asset, STypes.Asset memory a) external onlyOwner {
        ...
        //@dev Using HEAD's order contain oracle data
        ...
        _setInitialMargin(asset, a.initialMargin); // 50 -> 5 ether
        _setPrimaryLiquidationCR(asset, a.primaryLiquidationCR); // 40 -> 4 ether
        _setSecondaryLiquidationCR(asset, a.secondaryLiquidationCR); // 15 -> 1.5 ether
        _setForcedBidPriceBuffer(asset, a.forcedBidPriceBuffer); // 11 -> 1.1 ether
        _setMinimumCR(asset, a.minimumCR); // 11 -> 1.1 ether
    ...
    function setOracle(address oracle) external onlyOwner {
        s.oracle = oracle;
        emit Events.UpdateBaseOracle(oracle);
    }
    ...
    function setTithe(uint256 vault, uint16 zethTithePercent) external onlyOwner {
        _setTithe(vault, zethTithePercent);
        emit Events.ChangeVaultSetting(vault);
    }

    function setDittoMatchedRate(uint256 vault, uint16 rewardRate) external onlyOwner {
        _setDittoMatchedRate(vault, rewardRate);
        emit Events.ChangeVaultSetting(vault);
    }

    function setDittoShorterRate(uint256 vault, uint16 rewardRate) external onlyOwner {
    ...
    function setInitialMargin(address asset, uint16 value) external onlyOwner {
    ...
    function setPrimaryLiquidationCR(address asset, uint16 value) external onlyOwner {
        require(
    ...
    function setSecondaryLiquidationCR(address asset, uint16 value) external onlyOwner {
    ...
    function setForcedBidPriceBuffer(address asset, uint8 value) external onlyOwner {
        _setForcedBidPriceBuffer(asset, value);
        emit Events.ChangeMarketSetting(asset);
    }

    function setMinimumCR(address asset, uint8 value) external onlyOwner {
    ...
    function setResetLiquidationTime(address asset, uint16 value) external onlyOwner {
    ...
    function setSecondLiquidationTime(address asset, uint16 value) external onlyOwner {
    ...
    function setFirstLiquidationTime(address asset, uint16 value) external onlyOwner {
        _setFirstLiquidationTime(asset, value);
        emit Events.ChangeMarketSetting(asset);
    }

    function setTappFeePct(address asset, uint8 value) external onlyOwner {
        _setTappFeePct(asset, value);
        emit Events.ChangeMarketSetting(asset);
    }

    function setCallerFeePct(address asset, uint8 value) external onlyOwner {
        _setCallerFeePct(asset, value);
        emit Events.ChangeMarketSetting(asset);
    }

    function setMinBidEth(address asset, uint8 value) external onlyOwner {
        _setMinBidEth(asset, value);
        emit Events.ChangeMarketSetting(asset);
    }

    function setMinAskEth(address asset, uint8 value) external onlyOwner {
        _setMinAskEth(asset, value);
        emit Events.ChangeMarketSetting(asset);
    }

    function setMinShortErc(address asset, uint16 value) external onlyOwner {
    ...
    function deleteBridge(address bridge) external onlyOwner {
    ...
    function setWithdrawalFee(address bridge, uint16 withdrawalFee) external onlyOwner {
        _setWithdrawalFee(bridge, withdrawalFee);
        emit Events.ChangeBridgeSetting(bridge);
    }

    function setUnstakeFee(address bridge, uint8 unstakeFee) external onlyOwner {
        _setUnstakeFee(bridge, unstakeFee);
        emit Events.ChangeBridgeSetting(bridge);
    }
    ...
    function _setUnstakeFee(address bridge, uint8 unstakeFee) private {
        require(unstakeFee <= 250, "above 25.0");
